(() => {
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".tea-blends__slider")) new swiper_core_Swiper(".tea-blends__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                spaceBetween: 24,
                ...
                loop: true,
                navigation: {
                    prevEl: ".tea-blends__button-prev",
                    nextEl: ".tea-blends__button-next"
                },
                breakpoints: {
                    560: {
                        slidesPerView: 2,
...
***************************************************************************** */
        ...
        window.onload = function() {
            ...
            function documentActions(e) {
                const targetElement = e.target;
                if (targetElement.classList.contains("button-bag--big")) {
                    const productId = targetElement.closest(".tea-blends__item").dataset.pid;
                    addToCart(targetElement, productId);
                    e.preventDefault();
                }
                if (targetElement.classList.contains("tea-blends-list__delete")) {
                    const productId = targetElement.closest(".tea-blends-list__item").dataset.cartPid;
            ...
            function addToCart(productButton, productId) {
                if (!productButton.classList.contains("_hold")) {
                    ...
                    const cart = document.querySelector(".header__add-to-cart");
                    const product = document.querySelector(`[data-pid="${productId}"]`);
                    const productImage = product.querySelector(".tea-blends__image");
            ...
            function updateCart(productButton, productId, productAdd = true) {
                const cart = document.querySelector(".header__add-to-cart");
                const cartIcon = cart.querySelector(".header__link");
                const cartQuantity = cartIcon.querySelector("span");
                const cartProduct = document.querySelector(`[data-cart-pid="${productId}"]`);
                const cartList = document.querySelector(".tea-blends-list");
                if (productAdd) {
                    if (cartQuantity) cartQuantity.innerHTML = ++cartQuantity.innerHTML; else cartList.insertAdjacentHTML("beforeend", `<span>1</span>`);
                    if (!cartProduct) {
                        const product = document.querySelector(`[data-pid="${productId}"]`);
                        const cartProductImage = product.querySelector(".tea-blends__image").innerHTML;
                        const cartProductTitle = product.querySelector(".tea-blends__name").innerHTML;
                        const cartProductContent = `\n\t\t\t\t<a href="" class="tea-blends-list__image _ibg">${cartProductImage}</a>\n\t\t\t\t<div class="tea-blends-list__body">\n\t\t\t\t\t<a href="" class="tea-blends-list__title">${cartProductTitle}</a>\n\t\t\t\t\t<div class="tea-blends-list__quantity">Quantity: <span>1</span></div>\n\t\t\t\t\t<a href="" class="tea-blends-list__delete">Delete</a>\n\t\t\t\t</div>`;
                        cartList.insertAdjacentHTML("beforeend", `<li data-cart-pid="${productId}" class="tea-blends-list__item">${cartProductContent}</li>`);
                    } else {
                        const cartProductQuantity = cartProduct.querySelector(".tea-blends-list__quantity span");
